...
Resource.prototype.batch = function(requests, callback) {
  ...
  //alert(JSON.stringify(ajaxConfig))
...
// 以下为自定义函数,方便直接调用
function mcmFindX(app_id, callback) {
  var client = new Resource('A6068612489975', '2743CDE8-7662-B1CA-E22E-A1472A350D89');
  var Model = client.Factory("x");
  Model.query({
    filter: {
      where: {
        "app_id": app_id
...
function mcmAddX(data, callback) {
  var client = new Resource('A6068612489975', '2743CDE8-7662-B1CA-E22E-A1472A350D89');
  var Model = client.Factory("x");

  //查询app_id是否存在
  mcmFindX(data.app_id, function(ret, err) {
...
function mcmUpdateX(_id, data, callback) {
  var client = new Resource('A6068612489975', '2743CDE8-7662-B1CA-E22E-A1472A350D89');
  var Model = client.Factory("x");
  Model.save({ _id: _id }, data, function(ret, err) {
    alert('增加成功');
    callback(ret, err);
  });
}
...
function mcmFindWithId(_id, callback) {
  var client = new Resource('A6068612489975', '2743CDE8-7662-B1CA-E22E-A1472A350D89');
  var Model = client.Factory("x");
  Model.get({ "_id": _id }, function(ret, err) {
    callback(ret, err);
  });
}
